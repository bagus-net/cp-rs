...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <form action="{{ route('foto.add') }}" method="POST" role="form" enctype="multipart/form-data" id="myForm">
                    {{ csrf_field() }}
                    <div class="mb-3 row">
                        ...
                        <div class="col-md-10">
                            <select name="kategori_id" id="userSelectCategory" class="form-select" aria-label="Floating label select">
                                @foreach ($res_kategori_galeris as $item)
                                <option value="{{$item->id}}">{{$item->galeri_kategori}}</option>
                                @endforeach
                            </select>
                        </div>
                        <br><br>
